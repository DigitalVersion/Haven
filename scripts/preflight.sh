...
if [ "$skip_changelog" -eq 0 ]; then
    ...
    # Release-only for two reasons: it needs the network, and the thing it
    # protects — the checkupdates bot MR — is only generated by a release.
    # Not wrapped in run(): on failure the paths it names ARE the fix.
    ./scripts/check-fdroid-scandelete.sh || fail=1
else
    echo "· changelog gate skipped (--no-changelog)"
    echo "· F-Droid scandelete gate skipped (release-only)"
fi
